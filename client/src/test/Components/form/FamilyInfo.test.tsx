...
describe('FamilyInfo Component', () => {
  ...
  it('should render correctly', () => {
    render(<FamilyInfo formData={initialFormData} setFormData={mockSetFormData} />);
    ...
    expect(screen.getByLabelText(/পারিবারিক আর্থিক অবস্থা/i)).toBeInTheDocument();
  });
  ...
  it('should call setFormData when guardian information changes', () => {
    ...
    const fatherProfessionInput = screen.getByPlaceholderText(/পেশা , বিবরণসহ/i);
    const motherAliveStatusCheckbox = screen.getAllByLabelText(/জীবিত/i)[1];
    const motherProfessionInput = screen.getAllByPlaceholderText(/পেশা , বিবরণসহ/i)[1];
  ...
  it('should call setFormData when sibling information changes', () => {
    ...
    const brotherInfoInput = screen.getByPlaceholderText(/যেমনঃ ১ বড় ভাই, পেশা- ইঞ্জিনিয়ার, বিবাহিত/i);
    const sisterInfoInput = screen.getByPlaceholderText(/যেমনঃ ১ ছোট বোন, পেশা- ডাক্তার, বিবাহিত/i);
    const aboutSiblingsInput = screen.getByPlaceholderText(/যেমনঃ এক ভাই, এক বোন/i);

    // Change brother info
    fireEvent.change(brotherInfoInput, { target: { value: 'Brother is a doctor' } });
    expect(mockSetFormData).toHaveBeenCalledWith({
      ...initialFormData,
      siblings: { ...initialFormData.siblings, brotherInfo: 'Brother is a doctor' },
    });

    // Change sister info
    fireEvent.change(sisterInfoInput, { target: { value: 'Sister is a student' } });
    expect(mockSetFormData).toHaveBeenCalledWith({
      ...initialFormData,
      siblings: { ...initialFormData.siblings, sisterInfo: 'Sister is a student' },
    });

    // Change about siblings
    fireEvent.change(aboutSiblingsInput, { target: { value: 'We are a close family' } });
    expect(mockSetFormData).toHaveBeenCalledWith({
      ...initialFormData,
      siblings: { ...initialFormData.siblings, aboutSiblings: 'We are a close family' },
    });
  });
  ...
  it('should call setFormData when financial status changes', () => {
    ...
    const financialStatusSelect = screen.getByLabelText(/পারিবারিক আর্থিক অবস্থা/i) as HTMLSelectElement;
    fireEvent.change(financialStatusSelect, { target: { value: 'Middle Class' } });
