...
const PreferenceInfo: React.FC<PreferenceInfoProps> = ({ formData, setFormData }) => {
  const [localFormData, setLocalFormData] = useState<PreferenceInfoData>(formData);
      ...
      <div className="flex flex-col items-stretch md:items-center justify-center border border-gray-400 bg-purple-100 p-2 rounded-md md:m-4 shadow-lg">
        ...
        <form className="flex flex-col items-center justify-center bg-white border-pink-600 p-2 md:p-6 my-4 rounded-md border shadow-lg gap-4">
          {[
            ...
            { label: "আপনার অন্যান্য আরো কিছু শেয়ার করুন", name: "additionalInfo", placeholder: "যেকোনো অতিরিক্ত তথ্য শেয়ার করুন..." },
          ].map((field) => (
            <label key={field.name} className="w-full max-w-md flex flex-col items-center font-bold text-center p-2 text-sm md:text-xl text-cyan-950">
              {field.label}
              <textarea
                name={field.name}
