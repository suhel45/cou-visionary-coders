...
const LinkField: React.FC<LinkFieldProps> = ({ msgData, linkData, linkurl, onComplete, isGreen }) => {
  ...
  return (
    <div className="mt-4 bg-gray-100 p-4 rounded-md border-2 border-green-800">
      <p className="mt-4 text-lg sm:text-2xl text-green-900 font-bold">{msgData}</p>
      <button
...
const PrimaryProfile: React.FC<PrimaryProfileProps> = ({ username }) => {
  ...
  return (
    <div className="flex flex-col items-center p-6 rounded-md shadow-lg sm:w-1/3 text-center border border-gray-200 m-2">
      ...
      <div className={`flex items-center justify-center gap-2 mt-2 p-2 text-white rounded-md base-1/2 ${isVerified ? 'bg-green-600' : 'bg-indigo-900'}`}>
        <BadgeCheck className="text-white" />
        <span className="text-sm sm:text-lg font-semibold">Verification Status: 
          <span className={`text-white py-1 px-4 rounded-full m-2 ${isVerified ? 'bg-green-600' : 'bg-red-600'}`}>
            {isVerified ? 'Verified' : 'Unverified'}
          </span>
        </span>
      </div>
      <img src={profileimage} alt="Profile" className="w-1/2 h-auto rounded-full mt-4" />
      <div className="flex flex-col items-stretch gap-4 mt-4 w-full">
        {!isVerified && (
          <LinkField msgData="আপনার প্রোফাইল ভেরিফাই করুন" linkData="Verify Profile" linkurl="dashboard verify" onComplete={() => setIsVerified(true)} />
        )}
        { !isBiodataCreated && (
          <LinkField msgData="আপনার বায়োডাটা তৈরী করুন" linkData="Biodata Form" linkurl="dashboard edit biodata" onComplete={() => setIsBiodataCreated(true)} />
        )}
        {isBiodataCreated && (
          <LinkField msgData="আপনার বায়োডাটা দেখুন" linkData="View Biodata" linkurl="dashboard/view-biodata" onComplete={() => {}} isGreen={true} />
        )}
      </div>
    </div>
  );
