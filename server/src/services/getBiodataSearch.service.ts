...
export const getBiodataSearch = async (req: CustomReq) => {
    ...
    if (req.query.heightMin) {
      ...
      query['personalInfo.height']['$gte'] = parseInt(req.query.heightMin as string);
    }
    
    if (req.query.heightMax) {
      query['personalInfo.height'] = query['personalInfo.height'] || {};
      query['personalInfo.height']['$lte'] = parseInt(req.query.heightMax as string);
    }
    ...
    if (req.query.district) {
      ...
      query['$or'] = [
        { 'personalInfo.permanentAddress.district': districtRegex },
        { 'personalInfo.presentAddress.district': districtRegex }
      ];
    }
  
    // Financial Status filter
    if (req.query.financialStatus) {
      query['personalInfo.financialStatus'] = req.query.financialStatus;
    }
  
    // Education Level filter
    if (req.query.educationLevel) {
      query['personalInfo.educationLevel'] = req.query.educationLevel;
    }
